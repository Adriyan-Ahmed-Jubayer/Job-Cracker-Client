...
const MyJobs = () => {
    ...
    useEffect(() => {
        setTimeout(() => {
            axios.get(`https://job-cracker-bkvg9bh4v-adriyan.vercel.app/api/v1/collection?email=${User?.email}`)
    ...
    const handleDelete = (id) => {
        Swal.fire({
            ...
            if (result.isConfirmed) {
                fetch(`https://job-cracker-bkvg9bh4v-adriyan.vercel.app/api/v1/collection/${id}`, {
                    method: "DELETE"
                })
                .then(res => res.json())
                .then(data => {
